Ext.define('onlineOperation.verify.view.OtherController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.other',
    require:[
  ...
  onEnter:function(field,e){
    ...
    onInit:function(container, eOpts){
        ...
        var grid            = this.lookupReference('verifyOtherGrid');
        var otherDetails    = this.lookupReference('otherDetails');
        var verifyForm      = this.lookupReference('otherForm');
        var batchButton     = this.lookupReference('otherBatchButton');
        var otherReason     = this.lookupReference('otherReason');
        var otherAbroad     = this.lookupReference('otherAbroad');
        var otherTeach      = this.lookupReference('otherTeach');
        var otherExperience = this.lookupReference('otherExperience');
        var spokenScore     = this.lookupReference('spokenScore');

        grid.setWidth(w);
        grid.setHeight(h);
        otherDetails.setWidth((verifyDetails_w+40));
        otherDetails.setHeight((h+36));
        verifyForm.setWidth( document.body.scrollWidth-w-90 );
        batchButton.setMargin( '0 0 0 '+(w-70)+'' );
        otherReason.setWidth(verifyDetails_w);
        otherAbroad.setWidth(verifyDetails_w);
        otherTeach.setWidth(verifyDetails_w);
        otherExperience.setWidth(verifyDetails_w);
        spokenScore.setWidth(verifyDetails_w);
    }
