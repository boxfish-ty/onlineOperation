Ext.define('onlineOperation.pond.view.OtherController', {
    extend: 'Ext.app.ViewController',
    alias: 'controller.pondOther',
    require:[
    ...
    onAddFilter:function(){
        var pond_AddFilter  = (Ext.ComponentQuery.query('[itemId=pond_AddFilter]')[0]);
        var addFilterBtn    = (Ext.ComponentQuery.query('[itemId=addFilterBtn]')[0]);
        var collapsing      = pond_AddFilter.micro;

        pond_AddFilter.animate({dynamic: true,to: {height:collapsing?100:50}});
        pond_AddFilter.micro=(collapsing?false:true);
  ...
  onEnter:function(field,e){
    ...
    onInit:function(container, eOpts){
        ...
        var verifyDetails_w     =document.body.scrollWidth-w-120;
        var h                   =((document.body.scrollHeight*0.072)*10);

        var grid                = this.lookupReference('pondOtherGrid');
        var pond_Details        = this.lookupReference('pond_Details');
        var verifyForm          = this.lookupReference('pondOtherForm');
        var catch_Btn           = this.lookupReference('catch_Btn');
        var pondReason          = this.lookupReference('pondReason');
        var pondAbroad          = this.lookupReference('pondAbroad');
        var pondTeach           = this.lookupReference('pondTeach');
        var pondOtherExperience = this.lookupReference('pondOtherExperience');
        var pondSpokenScore     = this.lookupReference('pondSpokenScore');

        grid.setWidth(w);
        grid.setHeight(h);
        pond_Details.setWidth((verifyDetails_w+40));
        pond_Details.setHeight((h+36));
        verifyForm.setWidth( document.body.scrollWidth-w-90 );
        catch_Btn.setMargin( '0 0 0 '+(w-100)+'' );
        pondReason.setWidth(verifyDetails_w);
        pondAbroad.setWidth(verifyDetails_w);
        pondTeach.setWidth(verifyDetails_w);
        pondOtherExperience.setWidth(verifyDetails_w);
        pondSpokenScore.setWidth(verifyDetails_w);
    }
